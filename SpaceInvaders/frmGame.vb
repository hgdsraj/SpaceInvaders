Public Class frmGame
    ...
    Dim intMaxTotalAmountOFEnemies As Integer = 16 ' This is the limit for the amount of enemies
    ...
    Dim blnSpace As Boolean = False 'bln for no rapid fire, used twice
    Dim blnCollision As Boolean = False 'collision boolean, global so it  doesnt reset
    Dim blnBullets As Boolean = False 'boolean for bullet reload, used in various places
    Dim intMaxAmountOfBullets As Integer = 10 'max amount of bullets, global becuase it is used in various places
    Dim intLives As Integer = 4 ' hold amount of lives you have (3), global so it doesntreset
    ...
    Private Sub CollisonDetection()
        Dim intMaxScore As Integer = 1500 'maximum score
                            ...
                            If cltBullets(intLocalAmtOfBullets).Bounds.IntersectsWith(cltEnemies(intLocalAmountofEnemies).Bounds) Then 'bounds intersect for cltbullets and cltenemies
                                ...
                                If intScore = intMaxScore Then 'if intScore = intMaxScore then you win
                                    ...
                                    If intTime > intHighScore Then
                                        ...
                                        My.Settings.HighScore1 = intHighScore
                                        My.Settings.Save1 = 2
                                        My.Settings.Save()
                                    ...
                                    End If
                                    MessageBox.Show("This is level 2, enemies and enemy bullets are more frequent and faster, there are now 20 enemies which you have to destroy! Good luck, hit ok when ready!")
                                    Me.Hide() 'hides form
                                    frmGame2.Show() 'Shows Next Level!
    ...
    Private Sub mnuHighScore_Click(sender As System.Object, e As System.EventArgs) Handles mnuHighScore.Click 'tell user current high score
        MessageBox.Show("Current High Score = " & My.Settings.HighScore1)
    End Sub
    ...
    Private Sub tmrExplosions_Tick(sender As System.Object, e As System.EventArgs) Handles tmrExplosions.Tick 'when explosion timer ticks
